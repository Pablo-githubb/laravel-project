...
import { update as moviesUpdate } from '@/routes/movies';
...
/**
 * Pàgina per editar una pel·lícula existent.
 *
 * Mostra un formulari pre-emplenat amb les dades actuals
 * de la pel·lícula i gestiona la validació d'errors.
 */
export default function MoviesEdit({ movie }: { movie: Movie }) {
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(moviesUpdate(movie.id).url);
    }

    return (
        <PublicLayout title={`Editar: ${movie.title}`}>
            <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8 dark:border-neutral-700 dark:bg-neutral-800">
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4">
                        ...
                        >
                            {processing ? 'Actualitzant...' : 'Actualitzar Pel·lícula'}
                        </button>
                    </div>
                </form>
            </div>
